...
OUT_DIR="${OUT_DIR:-$BENCH_DIR/results/geometric_block1_$STAMP}"
K_VALUES="${K_VALUES:-2 4 6 8 10}"
BUDGETS="${BUDGETS:-36}"
ALPHA="${ALPHA:-0.8}"
R="${R:-0.8}"
MIN_FANOUT="${MIN_FANOUT:-1}"
...
echo "Geometric Block 1 real-LLM grid"
...
for budget in $BUDGETS; do
  for k in $K_VALUES; do
    fanouts="$(cd "$ROOT_DIR" && python -m sim.experiments.make_geometric_fanouts \
      --k "$k" \
      --budget "$budget" \
      --alpha "$ALPHA" \
      --r "$R" \
      --min-fanout "$MIN_FANOUT")"

    read -r -a fanout_array <<< "$fanouts"
    echo
    echo "k=$k budget=$budget fanouts=[$fanouts]"

    for offset in $PROMPT_OFFSETS; do
      run_name="geom_qwen${MODEL_SIZE}b_B${budget}_k${k}_a${ALPHA}_r${R}_n${NUMSEQS}_o${OUTPUT_LEN}_off${offset}"
